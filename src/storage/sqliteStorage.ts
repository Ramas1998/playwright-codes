...
export class SQLiteStorage implements StorageEngine {
  ...
  constructor(dbPath: string = "flaky-guardian.db") {
  ...
  async saveRun(run: StoredTestRun): Promise<void> {
  ...
  async getRun(runId: string): Promise<StoredTestRun | null> {
    ...
    const tests: StoredTestSummary[] = testRows.map((r) => ({
      testId: r.testId,
      title: r.title,
      ...
      duration: r.duration,
      statuses: JSON.parse(r.statuses) as TestStatus[],
  ...
  async getRunTests(runId: string): Promise<StoredTestSummary[]> {
    const run = await this.getRun(runId);
    return run?.tests ?? [];
  }

  async getRecentRuns(limit = 10): Promise<any[]> {
  ...
  async getFlakyTestsHistory(): Promise<any[]> {
    const rows = this.db
      .prepare(
        `
        SELECT
